...
# I loaded the 2D-mapped, clustered PBMC object from Phase 2. At this point, the clusters 
# only have numbers (0, 1, 2, etc.), and I need to figure out their actual biological identities.
pbmc <- readRDS("results/02_pbmc_clustered.rds")

# Step 2: Marker Gene Discovery (Differential Expression)
# I ran a Wilcoxon Rank Sum test across every single cluster to find genes that are highly expressed 
# in one cluster but turned off in all the others. These are the "biomarkers".
...
# I plotted these in a massive heatmap. I specifically forced the height to 16 inches during the save 
# so that the individual gene names are fully decompressed and legible on the Y-axis.
...
# Instead of manually guessing cell types by eyeballing marker genes (which is prone to human error), 
# I used the industry-standard `SingleR` package to algorithmically match my cells against the 
# Human Primary Cell Atlas database.
...
# This fully processed, mathematically clustered, and biologically annotated object is what 
# powers the final interactive dashboard!
