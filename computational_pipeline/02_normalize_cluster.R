...
# Raw RNA counts are highly skewed by sequencing depth (some cells get sequenced deeper than others). I normalized the data using LogNormalize (scaling by 10,000 to match standard transcripts-per-million rates).
pbmc <- NormalizeData(pbmc, normalization.method = "LogNormalize", scale.factor = 10000)

# Out of ~20,000 genes, most are just background noise. I ran a variance stabilizing transformation (VST) to isolate the top 2000 most highly variable genes that actually drive biological differences between cell types.
...
# I generated an Elbow Plot to visualize the drop-off in variance. I visually inspected this and determined that the vast majority of biological signal was captured in the first 10 PCs.
...
# I generated the UMAP plot. I specifically injected theme_classic() to override Seurat's default boxed aesthetic, applying professional X/Y axis arrows and bold titles for presentation-quality output.
